			...
			foreach (var operation in _operations)
			{
				switch (operation.Operation)
				{
					...
					case JsonPatchOperationType.test:
						string actualValue = PathHelper.GetValueFromPath(typeof(TEntity), operation.ParsedPath, entity).ToString();
						string testValue = operation.Value.ToString();
						if (testValue != actualValue)
						{
							throw new JsonPatchException($"Test operation failed at path {operation.Path} for value {operation.Value}");
						}
						break;
					default:
